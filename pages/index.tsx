...
const Main = () => {
    ...
    const [springs, api] = useTrail(WORD_TRANS.length, index => ({ from: { opacity: 0.1, x: 10 }, to: { opacity: 1, x: 0 }, loop: { reverse: true } }))
    ...
    return (
        <div>
            <Grid container direction="column" sx={{ alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Button variant="outlined" onClick={() => { dispatch(appSlice.actions.toggleTheme()) }}>{t('title.theme')}</Button>
            </Grid>
        </div >
    )
}
